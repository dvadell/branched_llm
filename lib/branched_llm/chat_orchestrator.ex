defmodule BranchedLLM.ChatOrchestrator do
  ...
  * `{:update_tool_usage_counts, counts}` — Updated tool usage counts for the
  ...
  defp handle_structured_output_tool_call(
    ...
    tool_call = Enum.find(tool_calls, &ToolCall.matches_name?(&1, synthetic_name))
    args = ToolCall.args_map(tool_call) || %{}
  ...
  defp structured_output_tool_call?(tool_calls) when is_list(tool_calls) do
    synthetic_name = Enforcer.structured_output_tool_name()
    Enum.any?(tool_calls, &ToolCall.matches_name?(&1, synthetic_name))
  end
